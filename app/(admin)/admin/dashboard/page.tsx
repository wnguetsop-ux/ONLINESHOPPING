...
const STEPS = [
  ...
  },
  {
    ...
    desc: 'Confirmation, rappel paiement, reçu — en 1 clic depuis la commande.',
...
export default function DashboardPage() {
  ...
  const [step, setStep]         = useState(0); // onboarding step actif
  ...
  // ── VUE NOUVEL UTILISATEUR (0 commandes) ─────────────────────────────────
  if (isNewUser) {
    return (
      <div className="max-w-xl space-y-5">
        ...
        <div className="rounded-2xl p-5 text-white"
          ...
          <p className="text-white/80 text-sm mt-2 leading-relaxed">
            Vous vendez sur WhatsApp ? Voici comment gérer vos commandes et clients en 3 étapes.
          </p>
        </div>
        ...
        <div className="space-y-3">
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={i}
                style={{ background: isActive ? s.bg : '#fff', border: `2px solid ${isActive ? s.border : '#f3f4f6'}` }}
                ...
                <button
                  ...
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0"
                    style={{ background: isActive ? s.bg : '#f9fafb', border: `2px solid ${isActive ? s.border : '#e5e7eb'}` }}>
                    {isDone ? '✅' : s.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold" style={{ color: isActive ? s.color : '#9ca3af' }}>
                        Étape {s.num}
                      </span>
                      {isDone && <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-bold">Fait ✓</span>}
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {i < STEPS.length - 1 && (
                        <button onClick={() => setStep(i + 1)}
                          className="px-4 py-2.5 rounded-xl text-sm font-medium text-gray-500 border border-gray-200 hover:bg-gray-50">
                          Étape suivante →
                        </button>
                      )}
                    </div>
        ...
        <Link href="/admin/orders"
          ...
          <div className="flex-1">
            <p className="font-black text-lg">Créer votre première commande</p>
            <p className="text-white/70 text-sm">Enregistrez une vente WhatsApp maintenant</p>
          </div>
        ...
        <div className="rounded-2xl p-4 bg-gray-50 border border-gray-100 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2 flex-shrink-0">
            <button onClick={copyLink} className="p-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
              {copied ? <CheckCircle className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4 text-gray-400" />}
            </button>
            <Link href={`/${shop?.slug}`} target="_blank" className="p-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { href: '/admin/products', emoji: '📦', label: 'Produits' },
            { href: '/admin/clients',  emoji: '👥', label: 'Clients' },
            { href: '/admin/settings', emoji: '⚙️', label: 'Réglages' },
          ].map((s, i) => (
            <Link key={i} href={s.href}
              className="rounded-xl p-3 flex flex-col items-center gap-1 bg-gray-50 hover:bg-gray-100 transition-colors text-center border border-gray-100">
  ...
  // ── VUE UTILISATEUR ACTIF (commandes existantes) ─────────────────────────
  return (
      ...
      {pending.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-600">Confirmez pour notifier vos clients sur WhatsApp</p>
          </div>
      ...
      {toRelance.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-green-600">En attente depuis plus de 2 jours</p>
          </div>
          <button onClick={() => toRelance[0] && quickRelance(toRelance[0])}
            disabled={!!sending}
