...
const RATES = {
  gemini: {
    ...
    image_per_call: 0.0002,
  },
  gpt4o: {
    input_per_1k:   0.0025,
    output_per_1k:  0.01,
    image_per_call: 0.00213,
  },
  firestore: {
    read_per_100k:   0.06,
    write_per_100k:  0.18,
...
function StripePaymentsTab({ shops }: { shops: any[] }) {
  ...
  return (
    <div className="space-y-6">
      {msg && (
        <div className="p-3 rounded-xl text-sm font-bold text-center bg-emerald-950 text-emerald-400 border border-emerald-800">
          {msg}
        </div>
      )}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 text-center">
          <p className="text-2xl font-extrabold text-violet-400">
            {confirmed.reduce((s: number, c: any) => s + c.credits, 0) +
             pending.filter((p: any) => p.status !== 'ACTIVATED').reduce((s: number, c: any) => s + c.credits, 0)}
          </p>
      ...
      <div className="bg-indigo-950 border border-indigo-800 rounded-2xl p-4 space-y-2">
        ...
        <p className="text-indigo-400 text-xs">
          ...
          Copie le &quot;Signing secret&quot; &rarr; ajoute dans .env.local &rarr; <span className="text-white font-bold">STRIPE_WEBHOOK_SECRET=whsec_xxx</span>
        </p>
      </div>
              ...
              <div key={p.id} className="bg-amber-950/40 border border-amber-800 rounded-2xl p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{p.email} &middot; {new Date(p.createdAt).toLocaleDateString('fr-FR')}</p>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    {shops.map((s: any) => (
                      <option key={s.id} value={s.id}>{s.name} &middot; /{s.slug}</option>
                    ))}
                  </select>
                  <button
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <select value={manualShopId} onChange={e => setManualShopId(e.target.value)}
            className="flex-1 bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-xs text-white">
          ...
          <select value={manualCredits} onChange={e => setManualCredits(e.target.value)}
          ...
          <button onClick={addManualCredits} disabled={activating === 'manual'}
        ...
        <button onClick={loadPayments} className="text-xs text-gray-500 hover:text-gray-300">Actualiser</button>
      </div>
    </div>
  );
}
  ...
  async function activatePlan(shopId: string, planId: string, months: number) {
    ...
    try {
      ...
      await updateDoc(doc(db, 'shops', shopId), {
        planId,
        planExpiry: months > 0 ? expiry.toISOString() : null,
      ...
      if (planId !== 'FREE' && months > 0) {
        ...
        await addDoc(collection(db, 'cost_events'), {
          ...
          type: 'subscription', planId, months,
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-amber-400 text-xs mt-1">
                    Firebase Console &rarr; Authentication &rarr; Sign-in methods &rarr; Google &rarr; Activer.
        ...
        {activeTab === 'traffic' && (() => {
          ...
          const funnelSteps = [
            { label: 'PageView - Arrivee sur le site',          count: pageViews,  color: '#6366f1' },
            { label: 'ViewContent - A lu la page (scroll 60%)', count: scrollReads, color: '#3b82f6' },
            { label: "AddToCart - A clique S'inscrire",         count: leads,      color: '#f59e0b' },
            { label: "Purchase - A finalise l'inscription",     count: purchases,  color: '#10b981' },
          ];
          ...
          return (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { label: 'PageViews',    val: pageViews,   color: 'text-indigo-400', bg: 'border-indigo-900', sub: 'Arrivees sur le site' },
                  { label: 'Lectures',     val: scrollReads, color: 'text-blue-400',   bg: 'border-blue-900',   sub: 'Ont scrolle 60%+' },
                  { label: 'Inscriptions', val: leads,       color: 'text-amber-400',  bg: 'border-amber-900',  sub: "Ont clique S'inscrire" },
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <h3 className="font-bold text-white mb-1">Entonnoir de conversion Facebook</h3>
                <p className="text-gray-500 text-xs mb-5">Ou perdez-vous vos visiteurs ?</p>
                <div className="space-y-3">
                  {funnelSteps.map((step, i) => {
                    const pct = Math.round((step.count / funnelMax) * 100);
                    const dropPct = i > 0 && funnelSteps[i-1].count > 0
                      ? Math.round(((funnelSteps[i-1].count - step.count) / funnelSteps[i-1].count) * 100) : 0;
                    return (
                      <div key={i}>
                        <div className="flex items-center justify-between text-xs mb-1.5">
                          ...
                          <div className="flex items-center gap-3">
                            {dropPct > 0 && (
                              <span className={`font-bold px-2 py-0.5 rounded text-[10px] ${dropPct > 70 ? 'text-red-400 bg-red-900/40' : dropPct > 40 ? 'text-amber-400 bg-amber-900/40' : 'text-green-400 bg-green-900/40'}`}>
                                -{dropPct}%
                              </span>
                            )}
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  { title: 'Abandon lecture',     pct: dropViewToRead, desc: 'Ont quitte sans lire la page',         tip: 'Chargement trop lent ou titre pas accrocheur' },
                  { title: 'Abandon inscription', pct: dropReadToCart, desc: "Ont lu mais pas clique S'inscrire",    tip: 'CTA pas assez visible ou offre peu claire' },
                  { title: 'Abandon formulaire',  pct: dropCartToBuy,  desc: 'Ont clique mais pas cree de compte',  tip: 'Formulaire trop long ou erreur technique' },
                ].map((a, i) => {
                  const p = parseInt(a.pct as string);
                  const cls = p > 70 ? 'text-red-400 bg-red-900/30 border-red-800' : p > 40 ? 'text-amber-400 bg-amber-900/30 border-amber-800' : 'text-green-400 bg-green-900/30 border-green-800';
                  return (
                    <div key={i} className={`rounded-2xl p-4 border ${cls}`}>
                      ...
                      {p > 40 && <p className="text-[10px] text-yellow-300 bg-black/30 rounded-lg p-2">{a.tip}</p>}
                    </div>
                  );
                })}
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-white">Visiteurs depuis Facebook Ads</h3>
                ...
                {fbEvents.length === 0 ? (
                  <div className="bg-blue-950/40 border border-blue-900/50 rounded-xl p-4 text-center">
                    <p className="text-blue-300 text-sm font-semibold">En attente de donnees Facebook</p>
                    <p className="text-blue-400/70 text-xs mt-1">Les visiteurs de vos pubs apparaitront ici</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {fbEvents.slice(0, 10).map((e: any, i: number) => (
                      <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-800 last:border-0">
                        <div className="w-7 h-7 bg-blue-900 rounded-full flex items-center justify-center text-xs font-bold">f</div>
                        <div className="flex-1 min-w-0">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${e.event === 'Purchase' ? 'bg-green-900 text-green-400' : e.event === 'Lead' ? 'bg-amber-900 text-amber-400' : 'bg-gray-800 text-gray-400'}`}>
                          {e.event || 'View'}
                        </span>
                      </div>
                    ))}
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <h3 className="font-bold text-white text-sm mb-4">Evolution - 14 derniers jours</h3>
        ...
        {activeTab === 'demo' && (() => {
          ...
          demoEvents.forEach((e: any) => {
            if (!sessionMap[e.sessionId]) {
              sessionMap[e.sessionId] = { sessionId: e.sessionId, device: e.device, referrer: e.referrer, firstSeen: e.createdAt, lastSeen: e.createdAt, tabs: [], converted: false };
            }
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-5">
                ...
                <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
                  ...
                  <div className="mb-4">
                    {[{label:'Mobile', pct:mobileRate, color:'bg-cyan-500'},{label:'Desktop', pct:100-mobileRate, color:'bg-blue-500'}].map((d,i) => (
                      <div key={i} className="mb-2">
                        <div className="flex justify-between text-xs text-gray-400 mb-1">
                          <span>{d.label}</span><span className="text-white font-semibold">{d.pct}%</span>
                        </div>
                        <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                  ...
                  <p className="text-gray-400 text-xs font-semibold uppercase tracking-wide mb-2">Provenance</p>
                  {(() => {
                    const refs: Record<string, number> = {};
                    demoEvents.filter((e: any) => e.action === 'open').forEach((e: any) => {
                      const r = e.referrer?.includes('facebook') ? 'Facebook' : e.referrer?.includes('instagram') ? 'Instagram' : e.referrer?.includes('whatsapp') ? 'WhatsApp' : e.referrer === 'direct' ? 'Direct' : 'Autre';
                      refs[r] = (refs[r] || 0) + 1;
                    });
                    return Object.entries(refs).sort((a,b) => b[1]-a[1]).slice(0,4).map(([r,c],i) => (
                      <div key={i} className="flex justify-between text-xs text-gray-400 py-1 border-b border-gray-800 last:border-0">
                        <span>{r}</span><span className="text-white font-semibold">{c} sessions</span>
                      </div>
                    ));
                  })()}
                </div>
                      ...
                      {sessionList.length === 0 ? (
                        ...
                        <tr key={i} className={`border-b border-gray-800/50 hover:bg-gray-800/30 ${s.converted ? 'bg-green-900/10' : ''}`}>
                          ...
                          <td className="px-4 py-2.5 text-gray-500">
                            {s.referrer?.includes('facebook') ? 'Facebook' : s.referrer?.includes('instagram') ? 'Instagram' : s.referrer?.includes('whatsapp') ? 'WhatsApp' : s.referrer === 'direct' ? 'Direct' : 'Autre'}
                          </td>
                          <td className="px-4 py-2.5">
                            {s.converted
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-800 text-gray-500 font-semibold">
                        {['Heure','Statut','Provider','Duree','Image','Produit','Erreur'].map(h => (
                    ...
                    <tbody>
                      {apiLogs.length === 0 && (
                        <tr><td colSpan={7} className="text-center text-gray-600 py-8">Les logs apparaitront ici des qu&apos;un utilisateur analyse une photo</td></tr>
                      )}
              ...
              <div className="bg-gray-900 border border-gray-700 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden">
                <div className="bg-gradient-to-r from-indigo-900 to-indigo-800 p-5 flex items-center justify-between">
                  ...
                  <button onClick={() => setRelanceShop(null)}
                    className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center text-white/70 hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="p-5 space-y-4">
                    ...
                    {waLink ? (
                      <a href={waLink} target="_blank" rel="noreferrer" onClick={() => setTimeout(() => setRelanceShop(null), 500)}
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-gray-400 text-[11px] mt-0.5">Message pre-rempli &middot; 1 clic</p>
                        </div>
                      </a>
                    ) : (
                      <div className="flex items-center gap-4 bg-gray-800 border border-gray-700 rounded-2xl p-4 opacity-50">
                        ...
                        <div>
                          ...
                          <p className="text-gray-600 text-xs">Numero non configure</p>
                        </div>
                      </div>
                    )}
                    ...
                    {emailLink ? (
                      <a href={emailLink} onClick={() => setTimeout(() => setRelanceShop(null), 500)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-gray-400 text-[11px] mt-0.5">Ouvre votre client mail</p>
                        </div>
                      </a>
                    ) : (
                      <div className="flex items-center gap-4 bg-gray-800 border border-gray-700 rounded-2xl p-4 opacity-50">
                        ...
                        <div>
                          ...
                          <p className="text-gray-600 text-xs">Email non disponible</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-gray-800 rounded-xl p-3 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-gray-500 mb-0.5">Lien boutique</p>
                      <p className="text-sm font-mono text-indigo-300 truncate">{shopUrl}</p>
                    </div>
                    <button onClick={() => navigator.clipboard.writeText(shopUrl)}
                      className="text-xs px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg flex-shrink-0">
                      Copier
                    </button>
                  </div>
              ...
              <div className="space-y-3">
                ...
                {(newEvent.type === 'gemini' || newEvent.type === 'gpt4o') && (
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { label: 'Tokens in',  key: 'tokens_in' as const },
                      { label: 'Tokens out', key: 'tokens_out' as const },
                      { label: 'Images',     key: 'images' as const },
                    ].map(f => (
                      <div key={f.key}>
                        <label className="text-xs text-gray-400 mb-1 block">{f.label}</label>
                        <input type="number" value={(newEvent as any)[f.key] || ''}
                ...
                {newEvent.type === 'subscription' && (
                  <div>
                    <label className="text-xs text-gray-400 mb-1 block">Revenu (FCFA)</label>
                    <input type="number" value={newEvent.revenue || ''}
                      onChange={e => setNewEvent({ ...newEvent, revenue: +e.target.value })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white focus:outline-none" />
                  </div>
                )}
                <div>
                  ...
                  <input type="text" value={newEvent.note || ''} onChange={e => setNewEvent({ ...newEvent, note: e.target.value })}
                    placeholder="Description..." className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="text-xs text-gray-400 mb-1 block">Date &amp; heure</label>
                  <input type="datetime-local" value={newEvent.createdAt?.slice(0,16) || ''}
                    onChange={e => setNewEvent({ ...newEvent, createdAt: e.target.value })}
                    className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500" />
                </div>
