                    ...
                    <tbody>
                      {sessionList.length===0?<tr><td colSpan={5} className="text-center text-gray-600 py-8">Aucune session</td></tr>:sessionList.slice(0,50).map((s:any,i:number)=>(
                        <tr key={i} className={`border-b border-gray-800/50 hover:bg-gray-800/30 ${s.converted?'bg-green-900/10':''}`}>
                          ...
                          <td className="px-4 py-2.5"><div className="flex gap-1 flex-wrap">{Array.from(new Set(s.tabs)).map((t:any,j:number)=><span key={j} className="bg-gray-700 text-gray-300 px-2 py-0.5 rounded text-[10px]">{t}</span>)}{s.tabs.length===0&&<span className="text-gray-600">accueil</span>}</div></td>
